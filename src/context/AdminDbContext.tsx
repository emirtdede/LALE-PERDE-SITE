...
import { fetchVisitorLogsPaginatedAction, fetchInboxPaginatedAction, fetchSearchLogsPaginatedAction } from '../app/admin/actions/analyticsActions';
import {
  Product,
  Category,
  SystemSettings,
...
export const AdminDbProvider: React.FC<{ children: React.ReactNode; initialData?: InitialDbData | null }> = ({ children, initialData }) => {
  ...
  const fetchVisitorLogsPaginated = async (page: number, limit: number = 50) => {
    const { data, error, count } = await fetchVisitorLogsPaginatedAction(page, limit);
  ...
  const fetchInboxPaginated = async (page: number, limit: number = 50) => {
    const { data, error, count } = await fetchInboxPaginatedAction(page, limit);
    if (error) { console.warn('Error fetching inbox', error); return { data: [], count: 0 }; }
  ...
  const fetchSearchLogsPaginated = async (page: number, limit: number = 50) => {
    const { data, error, count } = await fetchSearchLogsPaginatedAction(page, limit);
  ...
  return (
    <AdminDbContext.Provider value={contextValue}>
      {children}
    </AdminDbContext.Provider>
  );
};

export const useAdminDb = () => {
  const context = useContext(AdminDbContext);
  if (context === undefined) {
    throw new Error('useAdminDb must be used within a DbProvider');
  }
  return context;
};
